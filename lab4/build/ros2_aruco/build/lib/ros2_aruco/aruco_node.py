...
class ArucoNode(rclpy.node.Node):
    ...
    def image_callback(self, img_msg):
        ...
        if marker_ids is not None:
            ...
            # One marker at a time, so each can carry its own size. Unknown ids fall
            # back to the marker_size parameter instead of raising KeyError.
            for i, marker_id in enumerate(marker_ids):
                size = self.marker_size_map.get(marker_id[0], self.marker_size)

                rvec, tvec, _ = cv2.aruco.estimatePoseSingleMarkers(
                    [corners[i]], size, self.intrinsic_mat, self.distortion
                )

                rvecs.extend(rvec)
                tvecs.extend(tvec)
                final_marker_ids.append(marker_id)
